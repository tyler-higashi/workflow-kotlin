...
import com.squareup.benchmarks.performance.complex.poetry.PerformancePoetryActivity.Companion.EXTRA_RUNTIME_MULTIPLE_ACTIONS
...
class RenderPassTest {
  data class Scenario(
    ...
    val multipleActionsExpectation: RenderExpectation
  )
  ...
  @Test fun renderPassCounterBaselineComplexWithInitializingState() {
    runRenderPassCounter(COMPLEX_INITIALIZING, processMultipleActions = false)
  }

  @Test fun renderPassCounterBaselineComplexNoInitializingState() {
    runRenderPassCounter(COMPLEX_NO_INITIALIZING, processMultipleActions = false)
  }

  @Test fun renderPassCounterBaselineComplexNoInitializingStateHighFrequencyEvents() {
    runRenderPassCounter(COMPLEX_NO_INITIALIZING_HIGH_FREQUENCY, processMultipleActions = false)
  }

  @Test fun renderPassCounterMultipleActionsComplexWithInitializingState() {
    runRenderPassCounter(COMPLEX_INITIALIZING, processMultipleActions = true)
  }

  @Test fun renderPassCounterMultipleActionsComplexNoInitializingState() {
    runRenderPassCounter(COMPLEX_NO_INITIALIZING, processMultipleActions = true)
  }

  @Test fun renderPassCounterMultipleActionsComplexNoInitializingStateHighFrequencyEvents() {
    runRenderPassCounter(COMPLEX_NO_INITIALIZING_HIGH_FREQUENCY, processMultipleActions = true)
  }
  ...
  private fun runRenderPassCounter(
    ...
    val intent = Intent(context, PerformancePoetryActivity::class.java).apply {
      ...
      if (processMultipleActions) {
        putExtra(EXTRA_RUNTIME_MULTIPLE_ACTIONS, processMultipleActions)
      }
    ...
    val expectation =
      if (processMultipleActions) scenario.multipleActionsExpectation else
        scenario.baselineExpectation

    val title = if (processMultipleActions) {
      "Runtime: Multiple Actions w/ Timeout; "
    } else {
      "Runtime: Baseline; "
    } + scenario.title
  ...
  companion object {
    val COMPLEX_INITIALIZING = Scenario(
      ...
      multipleActionsExpectation = RenderExpectation(
        totalPasses = 41..42,
        freshRenderedNodes = 85..85,
    ...
    /**
     * Note that for this scenario the multiple actions runtime can have different results
